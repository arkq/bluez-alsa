...
static void *io_worker_routine(struct io_worker *w) {
	 ...
	 * resampler is used then this is the resampler output buffer; otherwise it
	 * is the same as the read_buffer. */
	...
	if (resampler_method != RESAMPLER_METHOD_NONE &&
		 ...
		 * convert to a rate natively supported by the soundcard.
		 * Initially also disable alsa-lib format conversion to try to use a
		 * format generated by the resampler. If that fails, then enable
		 * alsa-lib format conversion and have the resampler output its
		 * preferred FLOAT format; in this way the number of format conversions
		 * in the processing chain is minimized */
	...
	for (;;) {
		...
		case -1:
			...
			goto fail;
		case 0:
			if (!w->ba_pcm.running && ffb_len_out(&read_buffer) == 0)
		...
		if (!alsa_pcm_is_open(&w->alsa_pcm)) {
			...
			if (use_resampler) {
				resampler = resampler_create(
						resampler_method,
						w->ba_pcm.channels,
						resampler_native_format(pcm_format),
						w->ba_pcm.rate,
						w->alsa_pcm.format,
						w->alsa_pcm.rate,
				...
				ffb_init(&resampled_buffer, buffer_size,
							snd_pcm_format_size(w->alsa_pcm.format, 1));
				write_buffer = &resampled_buffer;
#if DEBUG
				if (verbose >= 5)
					debug("Resampler rate ratio: %.8f", resampler_current_rate_ratio(resampler));
#endif
			}
#endif
...
#if WITH_LIBSAMPLERATE
			snd_pcm_format_set_silence(resampler_native_format(w->alsa_pcm.format), read_buffer.data, ffb_len_out(&read_buffer));
#else
	...
	while ((opt = getopt_long(argc, argv, opts, longopts, NULL)) != -1)
		...
		case 10 /* --resampler */ : {
			static const nv_entry_t values[] = {
				...
				{ "medium", .v.ui = APLAY_CONV_CONV_SINC_MEDIUM_QUALITY },
				{ "fastest", .v.ui = APLAY_CONV_SINC_FASTEST },
				{ "zero-hold", .v.ui = APLAY_CONV_ZERO_ORDER_HOLD },
				{ "linear", .v.ui = APLAY_CONV_LINEAR },
				{ "none", .v.ui = RESAMPLER_METHOD_NONE },
				{ 0 },
			};
