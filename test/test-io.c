...
__attribute__ ((unused))
static const a2dp_lhdc_v2_t config_lhdc_v2_48000_stereo = {
...
#if ENABLE_LHDC
CK_START_TEST(test_a2dp_lhdc_v2) {

	struct ba_transport *t1 = test_transport_new_a2dp(device1,
			BA_TRANSPORT_PROFILE_A2DP_SOURCE, "/path/lhdc", &a2dp_lhdc_v2_source,
			&config_lhdc_v2_48000_stereo);
	struct ba_transport *t2 = test_transport_new_a2dp(device2,
			BA_TRANSPORT_PROFILE_A2DP_SINK, "/path/lhdc", &a2dp_lhdc_v2_sink,
			&config_lhdc_v2_48000_stereo);
	...
	if (aging_duration) {
		t1->mtu_read = t1->mtu_write = t2->mtu_read = t2->mtu_write = 2048;
...
#endif
#if ENABLE_LHDC
		{ a2dp_codecs_codec_id_to_string(A2DP_CODEC_VENDOR_ID(LHDC_V2_VENDOR_ID, LHDC_V2_CODEC_ID)), test_a2dp_lhdc_v2 },
