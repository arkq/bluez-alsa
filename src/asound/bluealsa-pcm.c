 ...
 * ioplug, can cause snd_pcm_avail() to return bogus values. This, in turn, can
 * trigger deadlock in applications built on the portaudio library (eg audacity)
 * and possibly cause faults in other applications too.
 * 
 ...
 * read-only. So in order to fix the problematic buffer size calculated by the
 * ioplug, we need to completely replace the hw_params container for the
 * bluealsa pcm.
 */
