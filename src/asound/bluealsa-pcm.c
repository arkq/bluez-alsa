...
/**
 * Helper function empties the PCM FIFO. */
static void clear_pcm_fifo(struct bluealsa_pcm *pcm) {
	splice(pcm->ba_pcm_fd, NULL, pcm->null_fd, NULL, pcm->delay_fifo_size * pcm->frame_size, SPLICE_F_NONBLOCK);
}
	...
	while (tframes < frames) {
		...
		if (pollret == -1) {
			...
			if (!pcm->fifo_active) {
				...
				if ((avail < (3 * pcm->io.period_size / 2)) && (pcm->io.period_size < pcm->delay_fifo_size)) {
 ...
 * @return true if transfer completed successfully, false if error occurred. */
static bool io_thread_read(struct bluealsa_pcm *pcm, snd_pcm_uframes_t offset, snd_pcm_uframes_t frames) {
	ssize_t ret = 0;
...
static int bluealsa_hw_params(snd_pcm_ioplug_t *io, snd_pcm_hw_params_t *params) {
	...
	pcm->delay_fifo_size = (unsigned) ret / pcm->frame_size;
...
static int bluealsa_prepare(snd_pcm_ioplug_t *io) {
	...
	if (io->stream == SND_PCM_STREAM_PLAYBACK) {
		...
		* transport is acquired, it does not wait for the Resume command. To
		* achieve the expected ALSA device behavior we therefore have to pause
		* the server, and discard any frames already sent. */
		if (!pcm->io_started) {
